...
func (r *SecurityconfigReconciler) Reconcile() (ctrl.Result, error) {
	...
	job = builders.NewSecurityconfigUpdateJob(
		...
		checksumval,
		adminCertName,
		r.determineOpenSearchServerCASecret(),
		cmdArg,
...
func (r *SecurityconfigReconciler) determineOpenSearchServerCASecret() string {
	caSecretName := ""
	if r.instance.Spec.Security != nil && r.instance.Spec.Security.Tls != nil {
		if helpers.SecurityChangeVersion(r.instance) {
			caSecretName = helpers.HTTPServerSecretRef(r.instance.Name, r.instance.Spec.Security.Tls.Http).Name
		} else {
			caSecretName = helpers.TransportServerCASecretRef(r.instance.Spec.Security.Tls.Transport).Name
		}
	}
	return caSecretName
}
