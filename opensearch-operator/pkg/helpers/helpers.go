...
// AdminCASecretRef returns the explicitly configured CA secret reference used to
// generate the admin certificate. OpenSearch 2.x uses the HTTP CA; earlier
// versions use the transport CA.
func AdminCASecretRef(cluster *opensearchv1.OpenSearchCluster) corev1.LocalObjectReference {
...
// HTTPEndpointCACertSecretRef returns the secret whose ca.crt is used to trust
// the OpenSearch HTTP endpoint.
func HTTPEndpointCACertSecretRef(clusterName string, tlsConfig *opensearchv1.TlsConfigHttp) corev1.LocalObjectReference {
	if tlsConfig == nil {
		return corev1.LocalObjectReference{}
	}
	if tlsConfig.Generate {
		return corev1.LocalObjectReference{Name: fmt.Sprintf("%s-http-cert", clusterName)}
	}
	return tlsConfig.Secret
}

// TransportEndpointCACertSecretRef returns the secret whose ca.crt is used to
// trust the OpenSearch transport endpoint.
func TransportEndpointCACertSecretRef(clusterName string, tlsConfig *opensearchv1.TlsConfigTransport) corev1.LocalObjectReference {
	if tlsConfig == nil {
		return corev1.LocalObjectReference{}
	}
	if tlsConfig.Generate {
		return corev1.LocalObjectReference{Name: fmt.Sprintf("%s-transport-cert", clusterName)}
	}
	return tlsConfig.Secret
}
